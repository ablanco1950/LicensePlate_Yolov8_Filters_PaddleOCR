...
@author: Alfonso Blanco, consulting IA Google
...
#########################################################################
    ...
    text, Accuraccy = GetPaddleOCR_VL(gray)
    ...
    if ProcessText(text) != "":
           ...
           if text==License:
              print(text + "  Hit without treatment " )
              TotHits=TotHits+1
           else:
    ...
    text, Accuraccy = GetPaddleOCR_VL(gray1)
    text = ''.join(char for char in text if char.isalnum())
    text=ProcessText(text)
    if ProcessText(text) != "":
    ...
    for z in range (10,11):
        ...
        text, Accuraccy = GetPaddleOCR_VL(img_concat)
        text = ''.join(char for char in text if char.isalnum())
        text=ProcessText(text)
        if ProcessText(text) != "":
    ...
    #cv2.waitKey()
    text, Accuraccy = GetPaddleOCR_VL(thresh)
    text = ''.join(char for char in text if char.isalnum())
    text=ProcessText(text)
    if ProcessText(text) != "":
    ...
    #cv2.waitKey()
    text, Accuraccy = GetPaddleOCR_VL(sharpened)
    text = ''.join(char for char in text if char.isalnum())
    text=ProcessText(text)
    if ProcessText(text) != "":
    ...
    text, Accuraccy = GetPaddleOCR_VL(o3)
